...
export function buildConversations(items: FeedItem[]): Conversation[] {
  ...
  for (const members of groups.values()) {
    ...
    out.push({
      ...
      members,
      root,
      score: Math.max(...members.map((m) => postScoreRate(m))),
      lastActivity: Math.max(...members.map((m) => timestampOf(m))),
      authors,
      dominantAuthor,
    })
  }
...
export function planView(convos: Conversation[], opts: PlanOpts): PlannedConvo[] {
  const { budget, autoUnrollMax = 10, perAuthorMax = 3, forceFull, ranking = 'mix' } = opts
